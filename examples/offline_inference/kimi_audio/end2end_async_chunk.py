...
"""Slice-3 example: Kimi-Audio audio-in / audio-out with async-chunk streaming.

The thinker (stage 0) emits audio tokens chunk-by-chunk; ``code2wav`` starts
synthesizing waveform as soon as the first chunk lands. End TTFB is bounded
by ``codec_chunk_frames`` from kimi_audio_async_chunk.yaml (25 frames * 480
samples / 24000 Hz ≈ 0.5 s of audio per chunk).
"""
...
def parse_args():
    parser = FlexibleArgumentParser(description="Kimi-Audio audio-in / audio-out streaming example.")
    ...
    parser.add_argument("--output-dir", default="./output_audio_streaming")
